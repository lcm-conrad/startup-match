...
export const ALL_SWITCHER_PAGES: readonly AppPage[] = [
  'auth', 'developer', 'admin', 'sprint', 'specform', 'milestone', 'analytics',
] as const
...
export function resolveRole(
  ...
  try {
    ...
    if (selectedRole === 'admin' || (isValidEmail && em.endsWith('@psits.org.ph'))) {
      return 'admin'
    }
    ...
    if (selectedRole === 'enterprise' || (isValidEmail && knownEnterprise.has(em))) {
      return 'enterprise'
    }
